...
export default function Messagerie({ user, userVisite, navigation }) {
    ...
    // --- 1. CHARGER LES MESSAGES AU DÉBUT ---
    useEffect(() => {
        if (userVisite) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-white flex flex-col">
            ...
            <div className="h-16 bg-blue-800 flex items-center px-4 shadow-lg sticky top-0 z-10">
                <button onClick={() => navigation("ProfilPublic")} className="mr-4 hover:bg-blue-700 p-2 rounded-full">←</button>
            ...
            <form onSubmit={handleSend} className="p-4 bg-slate-900 border-t border-slate-800 flex gap-2">
                ...
                />
                <button type="submit" className="bg-blue-600 hover:bg-blue-700 p-2 rounded-full w-10 h-10 flex items-center justify-center transition-colors">
                    ✈️
                </button>
            </form>
        </div>
